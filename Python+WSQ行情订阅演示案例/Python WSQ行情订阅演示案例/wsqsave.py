...
def myCallback(indata):
    ...
    for k in range(0,len(indata.Fields)):
         ...
         if(indata.Fields[k] == "RT_LATEST"):
             latestvalue = str(indata.Data[k][0]);
         if indata.Fields[k] == "RT_ASK1":
             ask1 = str(indata.Data[k][0])
         if indata.Fields[k] == "RT_BID1":
             bid1 = str(indata.Data[k][0])

    print(str(begintime), latestvalue, ask1, bid1)
    print("\n")
...
#订阅行情
w.wsq("P1607.DCE", "rt_time,rt_ask1,rt_bid1,rt_latest", func=myCallback)
while(1):
